...
import '../../app.dart';
import '../../models/CaptchaModel.dart';
...
class LoginController extends GetxController {
  ...
  var isOtpPasswordStatus = false.obs;
  var passObsureSignIn = false.obs;
  var enableConfirmPasswordFeild = false.obs;
  var passObsureSignUp = false.obs;
  var confirmPassObsureSignUp = false.obs;
  var registerIsVisible = false.obs;
  var registerIsVisibleCon = false.obs;
  // var termsCheckbox = false.obs;
 var  controllerForgetPassword=180.obs;
 var  controllerForgetPasswordD=180.0.obs;
  ...
  RxBool registerProgressIsVisible = false.obs;
  ...
  void apiGetCaptcha() async {
    captchaModel.value.data = null;
    Map<String, dynamic>? response =
        await WebServicesAuthHelper().apiGetCaptchaN();
  ...
  void loginApi(BuildContext context, String email, String password) async {
    Map<String, dynamic>? response =
        await WebServicesAuthHelper().apiLogin(email, password);
    HelperMethod().customPrint("check data ${response.toString()}");
    if (response != null) {
      debugPrint("access token ${response["data"]["accessToken"]}");
      HiveHelper.insertData("token", response["data"]["accessToken"]);
      var headerinfo = await WebServicesAuthHelper().getHeaderInfo();
      if (headerinfo != null) {
        HiveHelper.insertData("customer_id",
            headerinfo["data"]["customerAccounts"][0]["cust_aff_id"]);
      }
      CustomToast.showSuccessToast(msg: "Login Success");
      Navigator.pushNamed(context, Routes.dobScreen);
    }
  }

  void sendOtpApi(BuildContext context,String email) async {
    Map<String, dynamic>? response =
        await WebServicesAuthHelper().apiLoginWithOtp(email);
    HelperMethod().customPrint("check data ${response.toString()}");
  ...
  void verifyOtpLogin(BuildContext context, String email, String otp) async {
    //LoadingNetwork().showProgress();
    Map<String, dynamic>? response =
        await WebServicesAuthHelper().verifyOtpLogin(email, otp);
    HelperMethod().customPrint("check data ${response.toString()}");
    if (response != null && response["status"] == "Success") {
      ...
      Fluttertoast.showToast(msg: "Verify Otp Login Successfully");
  ...
  Future<void> getRegisterApi(BuildContext context,Map<String, dynamic> mapData) async {
    registerProgressIsVisible.value = true;
    Map<String, dynamic>? response =
        await WebServicesAuthHelper().apiRegister(mapData);
    HelperMethod().customPrint("check data ${response.toString()}");
    registerProgressIsVisible.value = false;
    if (response == null) {
      apiGetCaptcha();
    } else if (response!["statusCode"] == 200) {
      CustomToast.showSuccessToast(msg: "User Created Successfully");
  ...
  void resetPasswordApi(
    ...
    Map<String, dynamic>? response =
        await WebServicesAuthHelper().resetPassword(password, email);
    HelperMethod().customPrint("check data ${response.toString()}");

    if (response != null) {
      CustomToast.showSuccessToast(msg: "Password Changes Successfully");
