...
class CalanderView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
      ...
      final offset = DateUtils.firstDayOffset(month.year, month.month, MaterialLocalizations.of(context));
    final weekdays = ["S", "M", "T", "W", "T", "F", "S"];
    final nextMonth = DateUtils.addMonthsToMonthDate(month, 1);
    final endOffset = nextMonth.weekday == 7 ? 0 : 7 - nextMonth.weekday;
    final start = DateTime(month.year, month.month).subtract(Duration(days: offset));
    ...
    for (DateTime d = start; d.isBefore(end); d = d.add(const Duration(days: 1))) {
      final enabled = (d.isAfter(config.minDate) && d.isBefore(config.maxDate)) || DateUtils.isSameDay(d, config.minDate);
    ...
    return Column(
      children: [
        ...
        SizedBox(
          height: data.length/7>5?260.h:215.h,
